...
const ENGLISH_MESSAGES = Object.freeze({
  ...
  '传感器': 'Sensor',
  '场景对象': 'Scene Objects',
  'OBJ 定义三维形状，各 Mesh 的物化属性决定能量过程。树形 OBJ 可绑定光伏属性；板形 OBJ 可设为植被并绑定植被属性。光伏热电耦合使用 FacetEB，并需绑定不透明光谱。': 'OBJ defines the 3D geometry; each mesh material determines its energy process. Tree geometry can use PV materials; panel geometry can be classified as vegetation and use plant materials. PV coupling requires FacetEB and an opaque spectrum.',
  '修改对象物化属性会同步到全部 Mesh，也可逐行独立设置。OBJ 形状不限制光伏或植被用途；光伏热电耦合使用 FacetEB 和不透明光谱。': 'Changing the object material applies it to all meshes; individual rows remain editable. OBJ shape does not restrict PV or vegetation use. PV coupling requires FacetEB and an opaque spectrum.',
  '内置太阳能板 OBJ': 'Built-in Solar Panel OBJ',
  '单块倾斜单晶硅板': 'Single Tilted Monocrystalline Panel',
  '地面固定式阵列': 'Ground-mounted Fixed Array',
  '屋顶低倾角阵列': 'Low-tilt Rooftop Array',
  '垂直双面阵列': 'Vertical Bifacial Array',
  '太阳能电池板': 'Solar Panel',
  '太阳能板初始温度': 'Solar Panel Initial Temperature',
  '单晶硅太阳能板光谱': 'Monocrystalline PV Spectrum',
  '多晶硅太阳能板光谱': 'Polycrystalline PV Spectrum',
  '双面光伏组件光谱': 'Bifacial PV Module Spectrum',
  '单晶硅太阳能板（FacetEB）': 'Monocrystalline Solar Panel (FacetEB)',
  '多晶硅太阳能板（FacetEB）': 'Polycrystalline Solar Panel (FacetEB)',
  '双面太阳能板（FacetEB）': 'Bifacial Solar Panel (FacetEB)',
  ...
  '绘制立方体': 'Draw Box',
  '编辑 OBJ': 'Edit OBJ',
  '编辑场景对象': 'Edit Scene Objects',
  '编辑': 'Edit',
  '选择/编辑 OBJ': 'Select/Edit OBJ',
  '修改': 'Modify',
  '增加': 'Add',
  '选择对象': 'Select Object',
  '区域': 'Region',
  '立方体': 'Box',
  '选择并编辑场景中的 OBJ': 'Select and edit an OBJ in the scene',
  '移动': 'Move',
  '统一缩放': 'Uniform Scale',
  '在场景中拖画矩形区域并生成 OBJ': 'Drag a rectangular region in the scene and generate an OBJ',
  '在场景中拖画底面和高度并生成 OBJ': 'Draw a footprint and height in the scene and generate an OBJ',
  '在地面按住左键拖出矩形区域；松开后直接生成 OBJ': 'Drag on the ground with the left mouse button; release to generate the OBJ',
  '在地面拖出立方体底面；松开后移动鼠标设置高度，再单击确认': 'Drag the box footprint; release, move the pointer to set height, then click to confirm',
  '绘制范围太小，请重新拖动': 'The drawn area is too small; drag again',
  '旋转': 'Rotate',
  '缩放': 'Zoom',
  '平移': 'Pan',
  ...
  '面元计算后端': 'Facet Compute Backend',
  'GPU（默认）': 'GPU (Default)',
  'CPU（软件栅格）': 'CPU (Software Rasterizer)',
  '自动（GPU 失败回退 CPU）': 'Auto (Fall Back to CPU if GPU Fails)',
  'CPU 版保留软件栅格可见性、CSR 邻接图和 Jacobi 辐射度迭代；FacetEB 在该辐射解上继续计算温度与能量通量。CPU 适合兼容和验证，通常比 GPU 慢。': 'The CPU backend retains software-raster visibility, a CSR adjacency graph, and Jacobi radiosity iteration. FacetEB continues temperature and energy-flux calculations from that radiation solution. CPU is intended for compatibility and validation and is usually slower than GPU.',
  ...
  '辐射过程：短波、长波和净辐射；能量过程：潜热、显热、表面热通量、GPP 和 NPP。GPP 为叶片总光合，NPP 为叶片净同化近似值，暂未扣除根和茎呼吸。': 'Radiation outputs include shortwave, longwave, and net radiation. Energy outputs include latent heat, sensible heat, surface heat flux, GPP, and NPP. GPP is gross leaf photosynthesis; NPP is a leaf net-assimilation approximation that does not yet subtract stem or root respiration.',
...
const PHRASE_REPLACEMENTS = [
  ...
  ['太阳能板光谱', 'Solar Panel Spectrum'], ['建筑表面', 'Building Surface'],
