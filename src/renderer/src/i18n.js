...
const ENGLISH_MESSAGES = Object.freeze({
  ...
  '体元辐射-能量平衡': 'Voxel Radiative Transfer & Energy Balance',
  ...
  '采样数目': 'Sample Count',
  'GPU 序号': 'GPU Index',
  '临近区域数量': 'Neighboring Region Count',
  ...
  '波谱文件': 'Spectrum File',
  '亮度': 'Brightness',
  '统一方向：+X 向北、−X 向南；+Y 向上；+Z 向东、−Z 向西。OBJ、Three.js 与计算引擎采用同一约定；方位角 0° 向北、90° 向东；输出影像固定上北、右东。': 'Unified orientation: +X north, −X south; +Y up; +Z east, −Z west. OBJ, Three.js, and the simulation engine use the same convention. Azimuth is 0° north and 90° east; output images are north-up and east-right.',
  ...
  '0 自动关闭邻域；1–20 自动开启，仅填补传感器主视线越出研究区后的地表空缺。研究区内已经命中的像元、阴影和多次散射保持原计算不变，近水平观测建议设为 20。': '0 disables neighboring regions automatically; 1–20 enables them only to fill surface gaps where primary sensor rays leave the study area. Existing hits, shadows, and multiple scattering remain unchanged. Use 20 for near-horizontal views.',
  ...
  '导入 TXT': 'Import TXT',
  '支持空格、Tab 或逗号分隔；至少填写 X、Y，缺省值为 Z=当前高度、缩放=1、旋转角=0。': 'Supports spaces, tabs, or commas. Enter at least X and Z; defaults are Y=current height, scale=1, and rotation=0.',
...
const DYNAMIC_TRANSLATIONS = [
  ...
  [/^平行投影共\s*(\d+)\s*个观测方向。主平面批量范围为 −75°～75°、间隔 5°，同时包含太阳主平面和垂直太阳主平面；半球采用 128 个等面积方向。输出统一为上北、右东。$/u, 'Parallel projection provides $1 view directions. Principal-plane batches span −75° to 75° at 5° intervals and include both solar principal and perpendicular planes; hemispherical viewing uses 128 equal-area directions. Output is north-up and east-right.'],
