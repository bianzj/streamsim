...
用户侧与当前计算引擎统一采用右手坐标：`+X` 指向北、`-X` 指向南，`+Y` 垂直向上，`+Z` 指向东、`-Z` 指向西。太阳与视场方位角以北为 `0°`、东为 `90°`；输出影像固定为上北、右东。计算域由南北长度 `L_NS`、最大高度 `H` 和东西长度 `L_EW` 定义。
...
当前火焰只用于 VoxelRT/VoxelEB。用户给定一个三维火焰区域，程序将其体元化为具有温度、吸收、发射和可选散射特性的参与介质。沿路径 `s` 的形式解为：
