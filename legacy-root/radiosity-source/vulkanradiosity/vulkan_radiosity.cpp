...
VulkanRadiosity::~VulkanRadiosity()
{
    destroy();
}

void VulkanRadiosity::require(VkResult result, const char* operation) const
{
...
void VulkanRadiosity::initialize(const Config& config, const std::string& shaderDirectory)
{
    destroy();
...
void VulkanRadiosity::createContext()
{
...
void VulkanRadiosity::createCommandPool()
{
...
uint32_t VulkanRadiosity::findMemoryType(uint32_t typeBits,
...
VulkanRadiosity::Buffer VulkanRadiosity::createBuffer(VkDeviceSize size,
...
VkCommandBuffer VulkanRadiosity::beginCommands() const
{
...
void VulkanRadiosity::endCommands(VkCommandBuffer commandBuffer) const
{
...
void VulkanRadiosity::copyBuffer(VkBuffer source, VkBuffer destination, VkDeviceSize size) const
{
...
void VulkanRadiosity::uploadBuffer(Buffer& destination,
                                    const void* data,
...
void VulkanRadiosity::downloadBuffer(const Buffer& source,
                                      void* data,
                                      VkDeviceSize size) const
{
    if (size > source.size) {
...
void VulkanRadiosity::destroyBuffer(Buffer& buffer) const
{
...
VkShaderModule VulkanRadiosity::loadShader(const std::string& fileName) const
{
...
uint32_t VulkanRadiosity::nextPowerOfTwo(uint32_t value) const
{
    if (value <= 1U) {
        return 1U;
    }
...
uint32_t VulkanRadiosity::chooseHashCapacity() const
{
...
void VulkanRadiosity::validateGeometry(const std::vector<Vertex>& vertices) const
{
...
void VulkanRadiosity::setGeometry(const std::vector<Vertex>& vertices)
{
    if (!initialized()) {
...
std::array<float, 16> VulkanRadiosity::makeProjection(const Direction& direction) const
{
...
void VulkanRadiosity::createVisibilityResources()
{
...
void VulkanRadiosity::createVisibilityDescriptors()
{
...
void VulkanRadiosity::createVisibilityPipelines()
{
...
void VulkanRadiosity::createSolveResources(uint32_t directedEdgeCount)
{
...
void VulkanRadiosity::createSolveDescriptors()
{
...
void VulkanRadiosity::createSolvePipeline()
{
...
GraphDiagnostics VulkanRadiosity::buildVisibilityGraph(
...
std::vector<float> VulkanRadiosity::computeSunlitFraction(const Direction& sunDirection)
{
...
void VulkanRadiosity::validateOptics(const std::vector<SurfaceOptics>& optics) const
{
...
SolveResult VulkanRadiosity::solve(const std::vector<SurfaceOptics>& optics,
                                   float skyRadiosity,
                                   uint32_t iterations,
                                   float relaxation)
{
...
void VulkanRadiosity::destroySolveResources()
{
...
void VulkanRadiosity::destroyVisibilityResources()
{
    if (m_device == VK_NULL_HANDLE) {
        return;
    }
