...
function setRunning(running) {
  ...
  if (running) setEngineState('模拟运行中', `PID ${state.pid || '...'}`, 'running')
    ...
    const gpu = state.mode === 'eFacetRT'
    const exists = gpu ? state.radiosityExecutableExists : state.executableExists
    const path = gpu ? state.radiosityExecutable : state.executable
    setEngineState(exists ? (gpu ? 'GPU Radiosity 待命' : 'HiStream 待命') : '未找到引擎', path, exists ? 'ready' : '')
  }
}
...
function generateWorld(config = defaultConfig()) {
  ...
  const treeCount = Math.max(12, Math.min(90, config.objects.count ? config.objects.count * 10 : Math.round(width * depth / 25)))
  const trunks = new THREE.InstancedMesh(new THREE.CylinderGeometry(.09, .13, 1.2, 7), material(0x493e2b), treeCount)
  const crowns = new THREE.InstancedMesh(new THREE.IcosahedronGeometry(.65, 1), material(0x2f7856, .78), treeCount)
  trunks.castShadow = trunks.receiveShadow = crowns.castShadow = crowns.receiveShadow = true
  trunks.userData.kind = 'vegetation'; crowns.userData.kind = 'vegetation'
  const dummy = new THREE.Object3D()
  for (let i = 0; i < treeCount; i++) {
    let x = (seeded(i * 4) - .5) * width * .87
    const z = (seeded(i * 4 + 1) - .5) * depth * .87
    if (Math.abs(x) < width * .14 && Math.abs(z) < depth * .14) x += width * .24
    const scale = .7 + seeded(i * 4 + 2) * .85
    dummy.position.set(x, .6 * scale, z); dummy.scale.set(scale, scale, scale); dummy.updateMatrix(); trunks.setMatrixAt(i, dummy.matrix)
    dummy.position.y = 1.48 * scale; dummy.rotation.y = seeded(i * 4 + 3) * Math.PI; dummy.updateMatrix(); crowns.setMatrixAt(i, dummy.matrix)
  }
  world.add(trunks, crowns)
  const buildingMat = material(0x52645e, .67)
  const roofMat = material(0x263933, .75)
  const bw = Math.max(3.8, width * .13), bh = Math.max(3.2, width * .1), bd = Math.max(3.2, depth * .14)
  const building = new THREE.Mesh(new THREE.BoxGeometry(bw, bh, bd), buildingMat)
  building.position.set(width * .16, bh / 2, -depth * .12)
  building.castShadow = building.receiveShadow = true; building.userData.kind = 'building'; world.add(building)
  const roof = new THREE.Mesh(new THREE.ConeGeometry(Math.max(bw, bd) * .74, Math.max(.7, bh * .22), 4), roofMat)
  roof.position.set(building.position.x, bh + Math.max(.3, bh * .1), building.position.z); roof.rotation.y = Math.PI / 4; roof.castShadow = true; roof.userData.kind = 'building'; world.add(roof)
  const sensorBody = new THREE.Mesh(new THREE.BoxGeometry(.6, .25, .42), material(0x92bdb0, .35))
  sensorBody.position.set(-width * .17, Math.min(8, Math.max(4, width * .22)), depth * .12)
  sensorBody.castShadow = true; sensorBody.userData.kind = 'sensor'; world.add(sensorBody)
  const sensorRay = new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.ConeGeometry(2.1, sensorBody.position.y, 4, 1, true)), new THREE.LineBasicMaterial({ color: 0x62d9ab, transparent: true, opacity: .2 }))
  sensorRay.position.set(sensorBody.position.x, sensorBody.position.y / 2, sensorBody.position.z); sensorRay.rotation.y = Math.PI / 4; world.add(sensorRay)
  voxelPreview = new THREE.InstancedMesh(new THREE.BoxGeometry(.48, .48, .48), new THREE.MeshBasicMaterial({ color: 0x67e5b3, wireframe: true, transparent: true, opacity: .18 }), 280)
  for (let i = 0; i < 280; i++) {
    dummy.position.set((seeded(500 + i * 3) - .5) * width, .3 + seeded(501 + i * 3) * 3.6, (seeded(502 + i * 3) - .5) * depth)
    dummy.rotation.set(0, 0, 0); dummy.scale.set(1, 1, 1); dummy.updateMatrix(); voxelPreview.setMatrixAt(i, dummy.matrix)
  }
  voxelPreview.visible = $('#voxelBtn').classList.contains('active')
  world.add(voxelPreview)
...
function builtinNotice() { return state.inputPath ? '' : '<div class="property-group"><div class="notice">尚未打开工程。请点击“打开工程”，输入已有工程目录、project.json 或 Input.xml 的完整路径。</div></div>' }
...
function defaultDistribution(item = {}) {
  ...
  return {
    mode: vegetation ? 'random' : 'single', basis: 'count', count: vegetation ? 24 : 1, density: .02,
    rows: 4, columns: 6,
    minX: 0, maxX: sceneX,
    minY: 0, maxY: sceneY,
...
function distributionFromForm(form) {
  ...
  return {
    ...
    basis: values.distributionBasis === 'density' ? 'density' : 'count',
    count: Math.max(1, Math.min(10000, Math.round(finiteNumber(values.distributionCount, defaults.count)))),
    density: Math.max(.0001, finiteNumber(values.distributionDensity, defaults.density)),
    ...
    minX: finiteNumber(values.distributionMinX, defaults.minX), maxX: finiteNumber(values.distributionMaxX, defaults.maxX),
    minY: finiteNumber(values.distributionMinY, defaults.minY), maxY: finiteNumber(values.distributionMaxY, defaults.maxY),
...
function setDistributionForm(form, value) {
  ...
  const fields = {
    ...
    distributionCount: distribution.count, distributionDensity: distribution.density,
...
function renderModelParameters() {
  ...
  } else $('#modelParameterFields').innerHTML = '<div class="notice">自定义模型直接使用各波段反射率和透射率，多个波段用逗号分隔。</div>'
}
...
function renderPresetParameterFields(spectralModel = 'custom') {
  ...
  if (kind === 'spectrum') {
    ...
    target.innerHTML = `${presetSelect('光谱模型', 'presetSpectralModel', 'spectralModel', modelOptions)}${presetText('反射率', 'reflectance', spectralModel === 'Prospect' ? '0.08,0.45' : '0.20', 'required')}${presetText('透射率', 'transmittance', spectralModel === 'Prospect' ? '0.04,0.35' : '0.0', 'required')}${physicalInput('TIR 反射率', 'refTir', 0.05, 0.01)}${physicalInput('TIR 透射率', 'tauTir', 0, 0.01)}${parameters}`
...
async function saveImportedObject(event) {
  ...
  try {
    ...
    const distribution = { ...defaultDistribution({ type: values.objectType }), mode: 'single', basis: 'count', count: 1, scaleMin: 1, scaleMax: 1 }
...
function updateDistributionFormState(form) {
  ...
  form.elements.distributionCount.disabled = !usesPopulation || basis !== 'count'
  form.elements.distributionDensity.disabled = !usesPopulation || basis !== 'density'
...
function builtInOutputPath() {
  ...
  return state.platform === 'win32' ? `C:\\work\\histream\\examples\\${name}` : `/mnt/c/work/histream/examples/${name}`
}
...
function handleSimulationEvent(event) {
  if (event.type === 'started') { state.pid = event.pid; setRunning(true); addLog(`启动进程 PID ${event.pid}`, 'success'); addLog(event.command, 'command') }
    ...
    const engineName = state.mode === 'eFacetRT' ? 'GPU Radiosity' : 'HiStream'
    ...
    toast(success ? '模拟完成' : '模拟结束', success ? 'HiStream 已正常完成计算' : `退出码：${event.code}`, success ? '' : 'error'); setRunning(false)
    if (success && !$('#resultDialog').hidden) refreshResults(true).catch((error) => { $('#resultStatus').textContent = error.message })
  }
}
...
function drawEnviPreview(result, file) {
  const bandOptions = Array.from({ length: result.bands }, (_, index) => `<option value="${index}" ${index === result.band ? 'selected' : ''}>波段 ${index + 1}</option>`).join('')
  $('#resultPreview').innerHTML = `<div class="result-preview-card"><canvas class="result-canvas" id="resultCanvas" width="${result.previewWidth}" height="${result.previewHeight}"></canvas><div class="result-meta"><span>${result.width} × ${result.height}</span><span>${result.bands} BAND</span><span>MIN ${Number(result.minimum).toPrecision(6)}</span><span>MAX ${Number(result.maximum).toPrecision(6)}</span></div>${result.bands > 1 ? `<label class="result-band">显示波段<select class="select" id="resultBand">${bandOptions}</select></label>` : ''}</div>`
  ...
  for (let index = 0; index < source.length; index += 1) {
    const [red, green, blue] = resultColor(source.charCodeAt(index))
    const offset = index * 4
    image.data[offset] = red; image.data[offset + 1] = green; image.data[offset + 2] = blue; image.data[offset + 3] = 255
  }
  context.putImageData(image, 0, 0)
  $('#resultBand')?.addEventListener('change', (event) => openResultFile(file, Number(event.target.value)))
}
...
async function openResultFile(file, band = 0) {
  ...
  try {
    ...
    else if (result.kind === 'text') $('#resultPreview').innerHTML = `<pre class="result-text">${escapeHtml(result.content)}</pre>`
    else emptyResultPreview('暂不支持内置预览', '可在资源管理器中定位并使用外部程序打开该文件。')
    $('#resultStatus').textContent = result.kind === 'envi' ? `ENVI 浮点影像 · ${file.name}` : file.name
  } catch (error) {
...
function renderResultFiles() {
  ...
  $('#resultFiles').innerHTML = state.resultFiles.map((file, index) => {
    const extension = file.kind === 'envi' ? 'ENVI' : file.name.split('.').pop()
...
$('#voxelBtn').addEventListener('click', () => { voxelPreview.visible = !voxelPreview.visible; $('#voxelBtn').classList.toggle('active', voxelPreview.visible) })
...
$('#objectForm').addEventListener('input', () => updateDistributionFormState($('#objectForm')))
...
$('#engineBtn').addEventListener('click', async () => { if (state.mode === 'eFacetRT') { toast('GPU Radiosity 引擎', state.radiosityExecutable); return } const path = await api.chooseExecutable(); if (path) { state.executable = path; state.executableExists = true; setRunning(false); addLog(`HiStream 引擎：${path}`, 'success') } })
